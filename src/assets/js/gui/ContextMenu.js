import { findAncestor } from "../util/helper.js";
/**
 * HOW TO USE
 * 
...
class ContextMenu {
  ...
  turn(state, id) {
    if (this.collection.get(id).state != state) {    
      this.collection.get(id).state = state;
      this.display(id);
    }
  }
  ...
  #getDisplay(id) {
    return (this.collection.get(id).state ? this.collection.get(id).displayOnTrue : this.collection.get(id).displayOnFalse);
  }
